...
const LOGIN_URL = `http://account.ituring.com.cn/log-in?returnUrl=${encodeURIComponent(
  SHELF_URL
)}`
...
const printIturingBooks = async (userName, password, saveDir = './books/') => {
  ...
  try {
    ...
    for (let book of books) {
      ...
      const articles = await bookPage.$eval(
        '.bookmenu table tbody',
        element => {
          const articlesHTMLCollection = element.querySelectorAll('tr')
          const articlesElementArray = Array.prototype.slice.call(
            articlesHTMLCollection
          )
